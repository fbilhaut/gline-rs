...
pub struct RelexSpanPipeline<S, T> {
    splitter: S,
    tokenizer: T,
...
impl<'a, S: Splitter, T:Tokenizer> Pipeline<'a> for RelexSpanPipeline<S, T> {
    type Input = input::text::TextInput;
    type Output = output::decoded::RelexOutput;
    ...
    fn post_processor(&self, params: &Self::Parameters) -> impl PostProcessor<'a, Self::Output, Self::Context> {
        composed![
            ...
            output::decoded::span_relex::TensorsToDecoded::new(0.5, params.max_width), // TODO: threshold from config
            ...
            // TODO: relations
            // TODO: filter spans by threshold
        ]
    }
...
impl RelexSpanPipeline<crate::text::splitter::RegexSplitter, crate::text::tokenizer::HFTokenizer> {
...
pub type RelexSpanMode = RelexSpanPipeline<crate::text::splitter::RegexSplitter, crate::text::tokenizer::HFTokenizer>;


/// Specific GLiNER implementation using the default span-mode pipeline
impl super::super::GLiNER<RelexSpanMode> {
    pub fn new<P: AsRef<Path>>(params: params::Parameters, runtime_params: RuntimeParameters, tokenizer_path: P, model_path: P) -> Result<Self> {
        Ok(Self {            
            ...
            pipeline: RelexSpanPipeline::new(tokenizer_path)?,
            params,
        })
    }
    ...
    pub fn new_from_bytes(params: params::Parameters, runtime_params: RuntimeParameters, tokenizer_bytes: &[u8], model_bytes: &[u8]) -> Result<Self> {
        Ok(Self {
            ...
            pipeline: RelexSpanPipeline::new_from_bytes(tokenizer_bytes)?,
            params,
        })
    }
}